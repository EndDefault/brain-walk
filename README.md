...
초기 조건은 기억 20초·대기 3초·보기 4개·풀이 제한 없음입니다. **오프라인 Discounted UCB 밴딧 AI**가 유형별 같은 조건의 유효 10문제로 다음 난이도를 선택하고, 게임이 끝난 뒤 적용합니다. 새 설치에서 종합 게임 3회를 완료하면 세 유형이 각각 첫 10문제를 모읍니다. 잘 풀면 기억 시간·풀이 시간·보기 수·대기 시간이 규칙에 따라 조정되며, 어려워하면 단축 이력을 복원합니다. 연습은 초기 조건을 사용하며 AI를 학습시키지 않습니다.

학습 현황에서 현재 난이도와 다음 조정까지 모인 기록을 확인합니다. 미완성 묶음·적용 대기 결정·유형별 학습값은 종료 후에도 남습니다. 기존 v1 DB는 원본 기록을 보존해 v2로 옮기며, 유형별 최근 최대 10개 기록으로 최초 난이도를 보정합니다. 과거 기록으로 가상의 밴딧 보상을 만들지 않습니다. AI는 기기 내부의 게임 난이도 선택 알고리즘이며 외부 API·계정·인터넷 연결이 필요하지 않습니다.
...
app/src/main/
  java/com/example/memorysteps/
    ...
    data/                 Room v2·원본/묶음/결정 저장·종료 후 복구
...
클래식 메뉴·기록 보존 PR #4가 develop에 병합된 뒤 유형별 학습 묶음·난이도·AI를 연결했습니다. 실물 태블릿과 시니어 사용성 검증, 조건별 성과 비교·효과음/설정은 남아 있습니다. 개발 중인 PR은 `develop`을 대상으로 하며, 의존하는 앞 PR이 병합된 뒤 최신 `develop`에서 다음 브랜치를 만듭니다.
